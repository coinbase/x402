...
The `upto` scheme extends the base `SettlementResponse` with the actual settled amount:
...
This contract acts as the authorized Spender. It validates the Witness data to ensure the destination cannot be altered by the Facilitator, and supports settling for any amount up to the permitted maximum.
...
contract x402Permit2Proxy {
    ...
    // Struct to group EIP-2612 parameters and reduce stack depth
