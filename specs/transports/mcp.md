...
The server indicates payment is required using MCP's `isError: true` and the `_meta["x402/payment-required"]` field.

**Mechanism**: Tool result with `isError: true`, `_meta["x402/payment-required"]` set to `true` and `PaymentRequirementsResponse` in `structuredContent`
**Data Format**: `PaymentRequirementsResponse` in `result.structuredContent`
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "Payment required: _meta['x402/payment'] is required"
      }
    ],
    "isError": true,
    "structuredContent": {
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "Payment settlement failed: insufficient funds"
      }
    ],
    "isError": true,
    "structuredContent": {
...
| ---------------- | ----------------- | ------ | -------------------------------------------------- |
| Payment Required | Business Logic    | N/A    | `isError: true` with `PaymentRequirementsResponse` |
| Payment Failed   | Business Logic    | N/A    | `isError: true` with failure details               |
| Invalid Payment  | Protocol Error    | -32602 | Malformed payment payload or invalid parameters    |
| Server Error     | Protocol Error    | -32603 | Internal server error during payment processing    |
| Parse Error      | Protocol Error    | -32700 | Invalid JSON in payment payload                    |
| Method Error     | Protocol Error    | -32601 | Unsupported x402 method or capability              |

### Business Logic Errors (Payment-Related)

Payment-related errors use MCP's `isError: true` mechanism with `_meta["x402/payment-required"]` for fast detection:
