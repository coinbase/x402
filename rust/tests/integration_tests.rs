...
use x402::{
    client::{X402Client, DiscoveryClient},
    types::*,
    Result, X402Error,
};
...
async fn test_client_with_successful_payment() {
    ...
    let settlement_header = response.headers().get("X-PAYMENT-RESPONSE").unwrap();
    let settlement: SettleResponse = serde_json::from_slice(
        &base64::engine::general_purpose::STANDARD.decode(settlement_header.to_str().unwrap()).unwrap()
    ).unwrap();
...
async fn test_discovery_client() {
    ...
    assert_eq!(response.items[0].resource, "https://api.example.com/premium-data");
...
async fn test_payment_requirements_creation() {
    ...
    assert_eq!(amount_in_units, rust_decimal::Decimal::from_str("1.0").unwrap());
}
...
async fn test_payment_payload_serialization() {
    ...
    assert_eq!(payment_payload.payload.authorization.from, decoded.payload.authorization.from);
    assert_eq!(payment_payload.payload.authorization.to, decoded.payload.authorization.to);
}
...
async fn test_settle_response_serialization() {
    let settle_response = SettleResponse {
        success: true,
        error_reason: None,
        transaction: "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef".to_string(),
...
async fn test_error_handling() {
    ...
    let error = X402Error::NetworkNotSupported { network: "unsupported-network".to_string() };
