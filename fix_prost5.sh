...
# Let's fix observer-5d
sed -i 's/use arkhe_bridge::tree_validator::TreeManager;//g' cathedral-arkhe/observer-5d/src/lib.rs
sed -i 's/pub tree_manager: Arc<RwLock<TreeManager>>/pub tree_manager: Arc<RwLock<()>>/g' cathedral-arkhe/observer-5d/src/lib.rs
sed -i 's/tree_manager: Arc<RwLock<TreeManager>>,/tree_manager: Arc<RwLock<()>>,/g' cathedral-arkhe/observer-5d/src/lib.rs
sed -i 's/tm.trees.keys().cloned().collect::<Vec<_>>()/vec!["tree-001".to_string()]/g' cathedral-arkhe/observer-5d/src/lib.rs

# Let's fix council_grpc
cat << 'EOF2' > cathedral-arkhe/observer-5d/src/council_grpc.rs
use anyhow::Result;
