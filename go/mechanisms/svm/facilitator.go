...
func (f *ExactSvmFacilitator) Verify(
	...
	// Parse payload
	solanaPayload, err := PayloadFromMap(payload.Payload)
	if err != nil {
		return x402.VerifyResponse{
			IsValid:       false,
	...
	tx, err := DecodeTransaction(solanaPayload.Transaction)
	if err != nil {
		return x402.VerifyResponse{
			IsValid:       false,
			InvalidReason: "invalid_exact_solana_payload_transaction",
...
func (f *ExactSvmFacilitator) verifyComputePriceInstruction(tx *solana.Transaction, inst solana.CompiledInstruction) error {
	...
	if priceInst, ok := decoded.Impl.(*computebudget.SetComputeUnitPrice); ok {
		...
		if priceInst.MicroLamports > uint64(MaxComputeUnitPrice*1_000_000) {
...
func (f *ExactSvmFacilitator) verifyCreateATAInstruction(
...
func (f *ExactSvmFacilitator) confirmTransactionWithRetry(ctx context.Context, signature solana.Signature, network string) error {
