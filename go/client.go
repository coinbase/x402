...
// This is where the actual signing/wallet interaction happens
func (c *x402Client) CreatePaymentPayload(ctx context.Context, version int, requirements PaymentRequirements) (PaymentPayload, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	// Create the payment payload using the mechanism-specific client
	payload, err := client.CreatePaymentPayload(ctx, version, requirements)
	if err != nil {
...
func (c *x402Client) CreatePaymentForRequired(ctx context.Context, required PaymentRequired) (PaymentPayload, error) {
	...
	// Create payment
	return c.CreatePaymentPayload(ctx, required.X402Version, selected)
}
