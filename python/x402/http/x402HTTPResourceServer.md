# x402HTTPResourceServer - Implementation Plan

HTTP-enhanced resource server for protecting endpoints with x402 payments. Provides route-based configuration, payment verification, and settlement handling.

---

## Overview

The `x402HTTPResourceServer` wraps `x402ResourceServer` to provide:
- **Route Configuration** - Map URL patterns to payment requirements
- **Request Processing** - Extract and verify payments from HTTP requests
- **Response Building** - Create 402 responses with proper headers
- **Paywall Generation** - HTML paywall for browser requests
- **Settlement Handling** - Process settlements and add response headers

This class is framework-agnostic. Framework-specific middleware (FastAPI, Flask) adapts this class to their request/response models.

---

## File: `x402_http_server.py`
...
class x402HTTPResourceServer:
    ...
    def _compile_routes(self, routes: RoutesConfig) -> None:
        ...
        for pattern, config in normalized.items():
            ...
            self._compiled_routes.append(CompiledRoute(
                verb=verb,
                regex=regex,
                config=config,
            ))
    ...
    async def process_http_request(
        ...
        if payment_payload is None:
            ...
            if route_config.unpaid_response_body:
                unpaid_result = route_config.unpaid_response_body(context)
                if hasattr(unpaid_result, "__await__"):
                    unpaid_body = await unpaid_result
                else:
                    unpaid_body = unpaid_result
    ...
    async def _build_payment_requirements_from_options(
        ...
        for option in options:
            ...
            requirements = await self._server.build_payment_requirements_from_config(config)
    ...
    def _extract_payment(self, adapter: HTTPAdapter) -> PaymentPayload | None:
        ...
        header = (
            adapter.get_header("PAYMENT-SIGNATURE") or
    ...
    def _create_settlement_headers(
        ...
        # Include requirements in response for client reference
        response_with_reqs = settle_response.copy()
        response_with_reqs.requirements = requirements
        
        return {
            "PAYMENT-RESPONSE": encode_payment_response_header(response_with_reqs),
        }
    ...
    @staticmethod
    def _parse_route_pattern(pattern: str) -> tuple[str, re.Pattern]:
...
<body style="max-width: 600px; margin: 50px auto; padding: 20px; font-family: system-ui;">
    ...
    <div id="payment-widget" data-requirements='{html.escape(json.dumps(payment_required.to_dict()))}'>
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      x402HTTPResourceServer Flow                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌───────────┐   ┌───────────────────────────────────────────────────────┐  │
│  │  Request  │──▶│ process_http_request(context, paywall_config)        │  │
│  └───────────┘   └───────────────────────────────────────────────────────┘  │
│                                     │                                       │
│                                     ▼                                       │
│                       ┌─────────────────────────┐                           │
...
│                    ┌────────────────┴────────────────┐                      │
│                    ▼                                 ▼                      │
│            No matching route              Route found                       │
│                    │                                 │                      │
│                    ▼                                 ▼                      │
│         Return: NO_PAYMENT_REQUIRED     _extract_payment()                  │
│                                                      │                      │
│                                     ┌────────────────┴──────────────┐       │
│                                     ▼                               ▼       │
...
│                                     │                               │       │
│                                     ▼                               ▼       │
│                          Build 402 response          find_matching_reqs()   │
│                          (HTML or API)                              │       │
│                                     │               ┌───────────────┴───┐   │
│                                     ▼               ▼                   ▼   │
│                         Return: PAYMENT_ERROR   No match        verify()    │
│                                                     │               │       │
│                                                     ▼          ┌────┴────┐  │
│                                           Return: ERROR     Valid?       │  │
│                                                              ▼    ▼      │  │
...
└──────────────────────────────────────────────────────────────────────────┘  │
                                                                              │
│  After serving resource, call:                                              │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ process_settlement(payload, requirements)                             │  │
│  │   → settle_payment() → Returns headers to add to response             │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
- [ ] Route pattern matching (wildcards, params)
- [ ] Case-insensitive route matching
- [ ] Path normalization (query strings, slashes)
- [ ] Dynamic payTo/price resolution
- [ ] Payment extraction from headers
- [ ] Browser detection for paywall
- [ ] 402 response with PAYMENT-REQUIRED header
- [ ] Settlement header creation
...
from x402.http import x402HTTPResourceServer, RouteConfig, PaymentOption
from x402.http.middleware import create_fastapi_middleware
...
# Create server
server = x402ResourceServer(facilitator_client=facilitator)
http_server = x402HTTPResourceServer(server, routes)

# Use with FastAPI
from fastapi import FastAPI
app = FastAPI()
app.add_middleware(create_fastapi_middleware(http_server))
```
