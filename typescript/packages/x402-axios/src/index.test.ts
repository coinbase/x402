...
import { evm, PaymentRequirements, ChainIdToNetwork, Signer, MultiNetworkSigner } from "@b3dotfun/anyspend-x402/types";
...
describe("withPaymentInterceptor()", () => {
  ...
  it("should handle 402 errors and retry with payment header", async () => {
    ...
    const { createPaymentHeader, selectPaymentRequirements } = await import("@b3dotfun/anyspend-x402/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("@b3dotfun/anyspend-x402/types", async () => {
      const actual = await vi.importActual("@b3dotfun/anyspend-x402/types");
      return {
        ...actual,
    ...
    vi.doMock("@b3dotfun/anyspend-x402/client", () => ({
