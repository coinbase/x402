...
import { HTTPAdapter, x402HTTPResourceService, x402ResourceService } from "../../src/server";
import { buildCashPaymentRequirements, CashFacilitatorClient, CashSchemeNetworkClient, CashSchemeNetworkFacilitator, CashSchemeNetworkService } from "./mocks/cash";
...
describe("Core Integration Tests", () => {
  describe("x402Client / x402ResourceService / x402Facilitator - Cash Flow", () => {
    ...
    beforeEach(async () => {
      client = new x402Client()
        .registerScheme("x402:cash", new CashSchemeNetworkClient("John"))

      const facilitator = new x402Facilitator()
        .registerScheme("x402:cash", new CashSchemeNetworkFacilitator())
    ...
    it("server should successfully verify and settle a cash payment from a client", async () => {
      ...
      const selected = client.selectPaymentRequirements(paymentRequiredResponse.x402Version, accepts);
      const paymentPayload = await client.createPaymentPayload(paymentRequiredResponse.x402Version, selected)
  ...
  describe("x402HTTPClient / x402HTTPMiddleware / x402Facilitator - Cash Flow", () => {
    ...
    beforeEach(async () => {
      ...
      client = new x402HTTPClient().registerScheme("x402:cash", new CashSchemeNetworkClient("John")) as x402HTTPClient;
    ...
    it("middleware should successfully verify and settle a cash payment from an http client", async () => {
      ...
      const initial402Response = (await service.processHTTPRequest(context))!
      ...
      const paymentRequired = await client.getPaymentRequiredResponse(initial402Response.headers, initial402Response.body)
      const selected = client.selectPaymentRequirements(paymentRequired.x402Version, paymentRequired.accepts);
      const paymentPayload = await client.createPaymentPayload(paymentRequired.x402Version, selected);
      ...
      const responseInstructions = await service.processHTTPRequest(context);

      // No need to reason respond, can continue with request
      expect(responseInstructions).toBeNull();

      const settlementHeaders = await service.processSettlement(context, 200);
