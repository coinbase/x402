...
describe("resolveSettlementOverrideAmount", () => {
  ...
  describe("Hook Error Isolation", () => {
    ...
    beforeEach(() => {
      // Set up mock facilitator with default responses
      const supportedResponse = buildSupportedResponse({
        kinds: [{ x402Version: 2, scheme: "exact", network: "eip155:8453" as Network }],
      });
      facilitator = new MockFacilitatorClient(supportedResponse);
    ...
    describe("afterVerify hook errors", () => {
      it("should isolate afterVerify hook errors without affecting verification result", async () => {
        facilitator.setVerifyResponse({ isValid: true, payer: "0xPayer" });

        const hookErrorMessage = "Analytics service unavailable";
        server.onAfterVerify(async () => {
      ...
      it("should execute remaining afterVerify hooks even if one throws", async () => {
        facilitator.setVerifyResponse({ isValid: true, payer: "0xPayer" });
    ...
    describe("onVerifyFailure hook errors", () => {
      it("should isolate onVerifyFailure hook errors", async () => {
        facilitator.setVerifyResponse(new Error("Verification failed"));
        ...
        await expect(server.verifyPayment(payload, requirements)).rejects.toThrow(
          "Verification failed",
        );
    ...
    describe("afterSettle hook errors", () => {
      ...
      it("should preserve successful settlement despite multiple hook failures", async () => {
        facilitator.setSettleResponse({
          success: true,
          transaction: "0xSuccessfulTx",
          network: "eip155:8453",
        });
        ...
        const result = await server.settlePayment(payload, requirements);
        expect(result.success).toBe(true);
        expect(result.transaction).toBe("0xSuccessfulTx");
    ...
    describe("onSettleFailure hook errors", () => {
      it("should isolate onSettleFailure hook errors", async () => {
        facilitator.setSettleResponse(new Error("Settlement failed"));
        ...
        await expect(server.settlePayment(payload, requirements)).rejects.toThrow(
          "Settlement failed",
        );
      ...
      it("should not prevent recovery from successful onSettleFailure hooks", async () => {
        facilitator.setSettleResponse(new Error("Settlement failed"));
        ...
        server
          ...
          .onSettleFailure(async () => {
            // This hook should successfully recover
        ...
        // Only the first hook error should be logged (recovery stops execution)
        expect(consoleErrorSpy).toHaveBeenCalledTimes(1);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          "Error in onSettleFailure hook:",
          expect.objectContaining({
    ...
    describe("Critical settlement scenario", () => {
      it("should preserve successful settlement result even when afterSettle hooks fail", async () => {
        // This test specifically addresses the issue described in #1826:
        // afterSettle hooks run after successful on-chain settlement,
        // but errors were falling into onSettleFailure, making successful payments appear failed

        const successfulSettlement = {
          success: true,
          transaction: "0xSuccessfulOnChainTx",
          network: "eip155:8453" as const,
        };

        facilitator.setSettleResponse(successfulSettlement);

        // Simulate external service failures in afterSettle hooks
        server
          .onAfterSettle(async () => {
            throw new Error("External arbiter verification failed");
          })
          .onAfterSettle(async () => {
        ...
        // The settlement should return the successful result despite hook failures
        const result = await server.settlePayment(payload, requirements);
        expect(result).toEqual(successfulSettlement);

        // Hook errors should be logged but not affect the result
        expect(consoleErrorSpy).toHaveBeenCalledTimes(2);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          "Error in afterSettle hook:",
          expect.objectContaining({
            message: "External arbiter verification failed",
          }),
        );
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          "Error in afterSettle hook:",
