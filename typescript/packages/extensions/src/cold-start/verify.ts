...
export async function verifyColdStartSignalSignature(
  ...
  try {
    ...
    const payload = toArrayBuffer(
      options.payload ??
        options.canonicalize?.(signal) ??
        canonicalizeColdStartSignal(signal),
    );
...
function toArrayBuffer(value: string | Uint8Array): ArrayBuffer {
  const bytes = value instanceof Uint8Array ? Uint8Array.from(value) : new TextEncoder().encode(value);
