...
export interface CreateAttestationParams {
  ...
  settledAt?: number;
  /**
   * Unix timestamp when attestation expires (defaults to settledAt + 30 days)
   */
  validUntil?: number;
}
 ...
 * UTF8(payTo) || UTF8(payer) || uint64BE(settledAt) || uint64BE(validUntil))
 *
...
export function buildAttestationMessage(params: CreateAttestationParams): Uint8Array {
  ...
  const validUntil =
    params.validUntil ?? settledAt + 30 * 24 * 60 * 60; // Default: 30 days
  ...
  timestampView.setBigUint64(0, BigInt(settledAt), false); // false = big-endian

  // validUntil as 8-byte big-endian
  const validUntilBytes = new Uint8Array(8);
  const validUntilView = new DataView(validUntilBytes.buffer);
  validUntilView.setBigUint64(0, BigInt(validUntil), false);
  ...
  message.set(validUntilBytes, offset);

  return message;
}

/**
 * Hash the attestation message using SHA-256
 *
 * Note: In production, you should use keccak256 from @noble/hashes or similar.
 * This uses SHA-256 as a fallback that works in all environments.
 *
...
export async function createAttestation(
  ...
  const settledAt = params.settledAt ?? Math.floor(Date.now() / 1000);
  const validityPeriod = config.validityPeriod ?? 30 * 24 * 60 * 60; // Default: 30 days
  const validUntil = params.validUntil ?? settledAt + validityPeriod;

  // Build and hash the message
  const message = buildAttestationMessage({ ...params, settledAt, validUntil });
  ...
  return {
    ...
    facilitatorIdentity: config.facilitatorIdentity,
    settledAt,
    validUntil,
