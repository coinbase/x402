...
describe("Sign-In-With-X Extension", () => {
  ...
  describe("declareSIWxExtension", () => {
    it("should create extension with supportedChains and time-based fields", () => {
      ...
      // Time-based fields are generated directly by declareSIWxExtension
      // (overwritten by enrichPaymentRequiredResponse when used through the pipeline)
    ...
    it("should support multiple chains in single extension", () => {
      ...
      // Time-based fields are generated directly
      expect(extension.info.nonce).toBeDefined();
      expect(extension.info.nonce).toHaveLength(32);
      expect(extension._options.expirationSeconds).toBe(300);
    });
  ...
  describe("Integration - full signing and verification", () => {
    ...
    it("should work for auth-only endpoints (no enrichment)", async () => {
      ...
      // Server: declare challenge (auth-only, no enrichment pipeline)
...
describe("siwxResourceServerExtension", () => {
  ...
  it("should overwrite pre-generated nonce/issuedAt", async () => {
    const declaration = declareSIWxExtension({ expirationSeconds: 300 });
    const ext = declaration["sign-in-with-x"];

    // Capture the nonce generated by declareSIWxExtension
    const originalNonce = ext.info.nonce;
    expect(originalNonce).toBeDefined();
    expect(originalNonce).toHaveLength(32);
    ...
    // Enrichment should produce a fresh nonce, different from the original
    expect(result.info.nonce).toHaveLength(32);
    expect(result.info.nonce).not.toBe(originalNonce);
    expect(result.info.issuedAt).toBeDefined();
  });
});
