 ...
 * Uses the same portable crypto approach as the production code.
 *
...
function generateTimeBasedFields(expirationSeconds?: number) {
  // Use the same portable crypto approach as production code
...
describe("Sign-In-With-X Extension", () => {
  ...
  describe("Portable crypto generation", () => {
    it("should generate 32-character hex nonces using portable crypto", () => {
      const fields = generateTimeBasedFields();

      expect(fields.nonce).toHaveLength(32);
      expect(fields.nonce).toMatch(/^[0-9a-f]{32}$/);
    });

    it("should generate unique nonces", () => {
      const nonces = new Set();
      for (let i = 0; i < 100; i++) {
        nonces.add(generateTimeBasedFields().nonce);
      }
      expect(nonces.size).toBe(100); // All unique
    });

    it("should work in both Node.js and browser environments", () => {
      // Test that globalThis.crypto.getRandomValues is available
    ...
    it("should produce same format as production nonce generation", () => {
      // Simulate the exact production code path
      const productionNonce = Array.from(globalThis.crypto.getRandomValues(new Uint8Array(16)))
        .map(b => b.toString(16).padStart(2, "0"))
        .join("");

      const testNonce = generateTimeBasedFields().nonce;

      // Both should be 32-char hex strings
      expect(productionNonce).toMatch(/^[0-9a-f]{32}$/);
      expect(testNonce).toMatch(/^[0-9a-f]{32}$/);
      expect(productionNonce).toHaveLength(32);
      expect(testNonce).toHaveLength(32);
    });
  });
