...
import { ExactEvmService } from "../../src/exact";
import { MoneyParser } from "@x402/core/types";

describe("ExactEvmService - registerMoneyParser", () => {
  describe("Single custom parser", () => {
    it("should use custom parser for Money values", async () => {
      ...
      const result1 = await service.parsePrice(150, "eip155:1");
      ...
      const result2 = await service.parsePrice(50, "eip155:1");
    ...
    it("should receive decimal number, not raw string", async () => {
      ...
      await service.parsePrice("$1.50", "eip155:8453");
      expect(receivedAmount).toBe(1.5);
      expect(receivedNetwork).toBe("eip155:8453");

      await service.parsePrice("5.25", "eip155:8453");
      expect(receivedAmount).toBe(5.25);

      await service.parsePrice(10.99, "eip155:8453");
    ...
    it("should handle $ prefix removal before parsing", async () => {
      ...
      await service.parsePrice("$25.50", "eip155:8453");
    ...
    it("should not call parser for AssetAmount (pass-through)", async () => {
      ...
      const result = await service.parsePrice(assetAmount, "eip155:8453");
    ...
    it("should support async parsers with API calls", async () => {
      ...
      const result = await service.parsePrice(5, "eip155:8453");
    ...
    it("should fall back to default if parser returns null", async () => {
      ...
      const result = await service.parsePrice(1, "eip155:8453");
  ...
  describe("Multiple parsers - chain of responsibility", () => {
    it("should try parsers in registration order", async () => {
      ...
      await service.parsePrice(50, "eip155:8453");
    ...
    it("should stop at first non-null result", async () => {
      ...
      const result = await service.parsePrice(50, "eip155:8453");
    ...
    it("should handle mix of sync and async parsers", async () => {
      ...
      const result1 = await service.parsePrice(1500, "eip155:8453");
      expect(result1.asset).toBe("0xAsync");

      const result2 = await service.parsePrice(500, "eip155:8453");
  ...
  describe("Error handling", () => {
    it("should propagate errors from parser", async () => {
      ...
      await expect(async () => await service.parsePrice(50, "eip155:8453")).rejects.toThrow(
    ...
    it("should throw for invalid money format", async () => {
      ...
      await expect(
        async () => await service.parsePrice("not-a-number", "eip155:8453"),
    ...
    it("should throw for NaN values", async () => {
      ...
      await expect(async () => await service.parsePrice("abc123", "eip155:8453")).rejects.toThrow(
    ...
    it("should not catch errors - they should propagate", async () => {
      ...
      await expect(async () => await service.parsePrice(-5, "eip155:8453")).rejects.toThrow(
  ...
  describe("Real-world use cases", () => {
    it("should support tiered pricing by amount", async () => {
      ...
      const vipResult = await service.parsePrice(15000, "eip155:8453");
      ...
      const premiumResult = await service.parsePrice(5000, "eip155:8453");
      expect(premiumResult.extra?.tier).toBe("premium");
      expect(premiumResult.extra?.token).toBe("USDC-ETH");

      const standardResult = await service.parsePrice(500, "eip155:8453");
    ...
    it("should support dynamic exchange rates", async () => {
      ...
      const result = await service.parsePrice(100, "eip155:8453");
    ...
    it("should support custom token selection based on amount", async () => {
      ...
      const wethResult = await service.parsePrice(0.5, "eip155:8453");
      expect(wethResult.extra?.token).toBe("WETH");
      expect(wethResult.extra?.category).toBe("micro");

      const usdcResult = await service.parsePrice(100, "eip155:8453");
    ...
    it("should support network-specific logic in parser", async () => {
      ...
      const ethResult = await service.parsePrice(10, "eip155:1");
      expect(ethResult.extra?.network).toBe("ethereum");
      expect(ethResult.extra?.token).toBe("DAI");

      const baseResult = await service.parsePrice(10, "eip155:8453");
  ...
  describe("Multiple parsers - chain of responsibility", () => {
    ...
    it("should use default if all parsers return null", async () => {
      ...
      const result = await service.parsePrice(1, "eip155:84532");
    ...
    it("should support complex multi-tier logic", async () => {
      ...
      const tier1 = await service.parsePrice(15000, "eip155:8453");
      expect(tier1.extra?.tier).toBe(1);
      expect(tier1.extra?.token).toBe("DAI");

      const tier2 = await service.parsePrice(5000, "eip155:8453");
      expect(tier2.extra?.tier).toBe(2);
      expect(tier2.extra?.token).toBe("USDT");

      const tier3 = await service.parsePrice(500, "eip155:8453");
      expect(tier3.extra?.tier).toBe(3);
      expect(tier3.extra?.token).toBe("USDC");

      const tier4 = await service.parsePrice(50, "eip155:8453");
  ...
  describe("Integration with parsePrice flow", () => {
    it("should work with AssetAmount pass-through", async () => {
      ...
      service.registerMoneyParser(async () => {
    ...
    it("should work with all Money formats", async () => {
      ...
      await service.parsePrice("$10.50", "eip155:8453");
      await service.parsePrice("25.75", "eip155:8453");
      await service.parsePrice(42.25, "eip155:8453");
  ...
  describe("Edge cases", () => {
    it("should handle zero amounts", async () => {
      ...
      await service.parsePrice(0, "eip155:8453");
    ...
    it("should handle very small decimal amounts", async () => {
      ...
      await service.parsePrice(0.000001, "eip155:8453");
    ...
    it("should handle very large amounts", async () => {
      ...
      await service.parsePrice(999999999.99, "eip155:8453");
    ...
    it("should handle decimal precision correctly", async () => {
      ...
      const result = await service.parsePrice(1.123456789, "eip155:8453");
