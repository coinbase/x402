...
/**
 * Configuration options for RPC URL handling.
 */
export interface RpcConfig {
  /** Custom RPC URL for Solana devnet */
  solanaDevnetRpc?: string;
  /** Custom RPC URL for Solana mainnet */
  solanaMainnetRpc?: string;
}

/**
 * Creates a payment requirements selector with custom RPC configuration.
 *
 * @param rpcConfig - RPC configuration options
 * @param paymentRequirementsSelector - A function that selects the payment requirements from the response
 * @returns A selector with RPC configuration
 *
 * @example
 * ```typescript
 * const selector = withRpcConfig({
 *   solanaDevnetRpc: "http://localhost:8899",
 *   solanaMainnetRpc: "https://custom-mainnet-rpc.com"
 * });
 *
 * const fetchWithPay = wrapFetchWithPayment(fetch, wallet, maxValue, selector);
 * ```
 */
export function withRpcConfig(
  rpcConfig: RpcConfig,
  paymentRequirementsSelector: PaymentRequirementsSelector = selectPaymentRequirements,
): PaymentRequirementsSelector {
  return (paymentRequirements: PaymentRequirements[], network?: Network | Network[], scheme?: "exact") => {
    const selected = paymentRequirementsSelector(paymentRequirements, network, scheme);

    // Only add RPC config for Solana networks
    if (selected.network !== "solana" && selected.network !== "solana-devnet") {
      return selected;
    }
    ...
    if (selected.network === "solana-devnet" && rpcConfig.solanaDevnetRpc) {
      rpcUrl = rpcConfig.solanaDevnetRpc;
    } else if (selected.network === "solana" && rpcConfig.solanaMainnetRpc) {
      rpcUrl = rpcConfig.solanaMainnetRpc;
    }

    // If no custom RPC URL, return as-is
    if (!rpcUrl) {
      return selected;
    }

    // Return a new object with the RPC URL added to extra
    return {
      ...selected,
      extra: {
        ...selected.extra,
...
/**
 * Creates a payment requirements selector using localhost RPC for development.
 *
 * @param localRpcUrl - RPC URL for Solana Local Validator (defaults to "http://localhost:8899")
 * @returns A selector configured for local development
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function withLocalRpc(localRpcUrl = "http://localhost:8899"): PaymentRequirementsSelector {
  return withRpcConfig({
    solanaDevnetRpc: localRpcUrl,
  });
}
