...
describe("RPC Helper Functions", () => {
  ...
  describe("getRpcClientFromRequirements", () => {
    it("should use default RPC URL when not provided in requirements", () => {
      const mockRpcClient = { mock: "default-client" };
      vi.mocked(solanaKit.createSolanaRpc).mockReturnValue(mockRpcClient as any);
      ...
      const result = getRpcClientFromRequirements(paymentRequirements);

      expect(solanaKit.devnet).toHaveBeenCalledWith("https://api.devnet.solana.com");
      expect(result).toBe(mockRpcClient);
    });

    it("should use custom RPC URL from requirements.extra.rpcUrl (devnet)", () => {
      const mockRpcClient = { mock: "custom-client" };
      const customUrl = "http://localhost:8899";
      vi.mocked(solanaKit.createSolanaRpc).mockReturnValue(mockRpcClient as any);
    ...
    it("should use custom RPC URL from requirements.extra.rpcUrl (mainnet)", () => {
      const mockRpcClient = { mock: "custom-client" };
      const customUrl = "http://localhost:8899";
      vi.mocked(solanaKit.createSolanaRpc).mockReturnValue(mockRpcClient as any);
  ...
  describe("getRpcSubscriptionsFromRequirements", () => {
    it("should use default WSS URL when not provided in requirements", () => {
      const mockSubscriptions = { mock: "default-subscriptions" };
      vi.mocked(solanaKit.createSolanaRpcSubscriptions).mockReturnValue(mockSubscriptions as any);
      ...
      const result = getRpcSubscriptionsFromRequirements(paymentRequirements);

      expect(solanaKit.devnet).toHaveBeenCalledWith("wss://api.devnet.solana.com");
      expect(result).toBe(mockSubscriptions);
    });

    it("should use custom WSS URL from requirements.extra.rpcUrl (devnet)", () => {
      const mockSubscriptions = { mock: "custom-subscriptions" };
      const customUrl = "wss://localhost:8900";
      vi.mocked(solanaKit.createSolanaRpcSubscriptions).mockReturnValue(mockSubscriptions as any);
    ...
    it("should use custom WSS URL from requirements.extra.rpcUrl (mainnet)", () => {
      const mockSubscriptions = { mock: "custom-subscriptions" };
      const customUrl = "wss://localhost:8900";
      vi.mocked(solanaKit.createSolanaRpcSubscriptions).mockReturnValue(mockSubscriptions as any);
