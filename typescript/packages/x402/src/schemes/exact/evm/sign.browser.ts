...
/**
 * Gets random values using the browser's crypto API
 */
 ...
 */
export async function signPaymentMessage(
  ...
  const message = `Authorize payment of ${paymentRequirements.maxAmountRequired} ${paymentRequirements.asset} to ${paymentRequirements.payTo} for ${paymentRequirements.resource}`;
