...
* **EVM**: Any ERC-20 token that implements the EIP-3009 standard, or any ERC-20 token via Permit2 with gas sponsoring extensions
...
#### EVM: Permit2 with Gas Sponsoring Extensions

For tokens that don't implement EIP-3009, x402 supports Permit2-based payments with gas sponsoring extensions:

* **[EIP-2612 Gas Sponsoring](/extensions/eip2612-gas-sponsoring)**: For tokens that implement EIP-2612 (like USDC, DAI). The facilitator submits the permit on-chain, enabling gasless approvals.
* **[ERC-20 Approval Gas Sponsoring](/extensions/erc20-approval-gas-sponsoring)**: For any ERC-20 token. The client signs an approval transaction (without broadcasting), and the facilitator broadcasts it before settling the payment.

These extensions enable gasless payments for a wider range of tokens beyond EIP-3009.
