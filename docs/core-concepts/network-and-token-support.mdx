...
| `algorand` | `algorand:wGHE2Pwdvd7S12BL5FaOP20EGYesN73ktiC1qzkkit8=` | - | Algorand Mainnet |
...
- **Algorand**: `algorand:<genesisHash>` where genesisHash is the base64-encoded genesis hash
...
x402 supports tokens on EVM, Solana, and Algorand networks:

* **EVM**: Any ERC-20 token that implements the EIP-3009 standard
* **Solana**: Any SPL or token-2022 token
* **Algorand**: Any Algorand Standard Asset (ASA)

**Important**: Facilitators support networks, not specific tokens — any EIP-3009 compatible token works on EVM networks, any SPL/token-2022 token works on Solana, and any ASA works on Algorand, for the facilitators that support those networks.
...
#### Algorand: Algorand Standard Assets (ASA)

On Algorand, x402 supports all Algorand Standard Assets (ASA), which are native assets of the Algorand protocol requiring no smart contracts. When using facilitators that support Algorand, payments can be made in any ASA by specifying the ASA ID (64-bit unsigned integer) in the `asset` field. The `exact` scheme on Algorand uses atomic transaction groups to enable gasless payments where the facilitator can sponsor transaction fees.
