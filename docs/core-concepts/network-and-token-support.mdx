...
- **Stellar**: `stellar:<network>` where network is the network identifier (e.g., `testnet`)
...
* **Supports**: `eip155:84532` (Base Sepolia), `solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1` (Solana Devnet), `stellar:testnet` (Stellar Testnet)
...
* **Stellar**: Any Stellar asset
* **Aptos**: Any fungible asset using Aptos's native fungible asset framework

**Important**: Facilitators support networks, not specific tokens — any EIP-3009 compatible token works on EVM networks, any SPL/token-2022 token works on Solana, any Stellar asset works on Stellar, and any fungible asset works on Aptos, for the facilitators that support those networks.
...
On Stellar, x402 supports all Stellar assets. When using facilitators that support Stellar Testnet, payments can be made in any Stellar asset. The TypeScript SDK supports sponsored transactions where facilitators pay gas fees on behalf of clients.
...
| ---------------- | ------------------ | ---------------- | ------------ |
| x402.org (Default) | `eip155:84532`, `solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1`, `stellar:testnet` | Testnet only | None |
| [Production Facilitators](https://www.x402.org/ecosystem?filter=facilitators) | Various (Base, Solana, Stellar, Polygon, Avalanche, etc.) | Yes | Varies |
| Self-hosted | Any EVM network (CAIP-2 format) | Yes | Technical setup |

**Note**: On EVM networks, facilitators support any EIP-3009 compatible token; on Solana, facilitators support any SPL/Token-2022 token; on Stellar, facilitators support any Stellar asset.
...
* Base (`eip155:8453`), Base Sepolia (`eip155:84532`), Solana (`solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp`), Solana Devnet (`solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1`), and Stellar Testnet (`stellar:testnet`) have the best out-of-the-box support
