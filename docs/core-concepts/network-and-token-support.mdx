...
* **Solana**: Any SPL token (v1 & v2); Token-2022 (v2 only)
...
On Solana, x402 supports all SPL tokens for v1 and v2. Token-2022 tokens are supported in v2 only. When using facilitators that support Solana or Solana Devnet, payments can be made in any SPL token (or Token-2022 for v2), including USDC (SPL). No EIP-712 configuration is required on Solana.
