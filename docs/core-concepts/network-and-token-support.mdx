...
#### EVM: Payment Authorization Methods

x402 supports two methods for authorizing EVM token transfers:

**1. EIP-3009 (Recommended for compatible tokens)**

Tokens that implement the `transferWithAuthorization` function from the EIP-3009 standard. This enables:
...
* **One-step flow**: No approval transaction required

**2. Permit2 (Universal fallback for any ERC-20)**

Uses Uniswap's [Permit2](https://github.com/Uniswap/permit2) contract for universal ERC-20 support:

* **Works with any ERC-20**: No special token implementation required
* **Two-step flow**: Requires one-time approval transaction, then gasless signatures
* **Secure witness pattern**: Enforces payment destination via smart contract
* **Canonical deployment**: Same Permit2 address (`0x000000000022D473030F116dDEE9F6B43aC78BA3`) on all chains

The client automatically uses EIP-3009 by default. Facilitators can specify Permit2 via the `assetTransferMethod` field in payment requirements.
...
#### Choosing the Transfer Method (Server-Side)

Facilitators can specify which authorization method to use via the `assetTransferMethod` field in payment requirements:
...
- **EIP-3009** (default): Use for USDC and other tokens with native `transferWithAuthorization` support. Provides the best user experience with no approval transaction.
- **Permit2**: Use for any ERC-20 token that doesn't implement EIP-3009. Requires a one-time approval but works universally.

The client SDK automatically handles both methods based on the facilitator's requirements.
...
#### Using Permit2 with Any ERC-20 Token

For tokens that don't implement EIP-3009, x402 supports Permit2 as a universal fallback. This requires a one-time approval transaction before the first payment:
...
<Tabs>
  <Tab title="TypeScript">
    ...
    if (allowance < requiredAmount) {
      ...
      // Wait for confirmation before making payments
    }

    // After approval, payments work automatically via Permit2
    ```
  </Tab>
</Tabs>

**Note**: The approval transaction grants Permit2 unlimited allowance (`type(uint256).max`). This is a one-time cost paid by the user. After approval, all subsequent payments are gasless signatures.

#### Why EIP-3009 and Permit2?

These standards are essential for x402 because they enable:

1. **Gas abstraction**: Buyers don't need native tokens (ETH, MATIC, etc.) for gas
2. **Signature-based authorization**: Payments via off-chain signatures
3. **Universal token support**: EIP-3009 for compatible tokens, Permit2 for any ERC-20
4. **Secure transfers**: Cryptographic authorization prevents unauthorized payments
