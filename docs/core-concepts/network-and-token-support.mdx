...
**Important**: Facilitators support networks, not specific tokens — any ERC-20 token works on EVM networks (using EIP-3009 or Permit2), and any SPL/token-2022 token works on Solana, for the facilitators that support those networks.

#### EVM: Payment Methods

x402 supports two payment methods on EVM networks:
...
* **No approval required**: Works immediately without prior token approval

**Permit2 (Alternative)**

Uses Uniswap's [Permit2 contract](https://github.com/Uniswap/permit2) for token transfers. This enables:

* **Gasless transfers**: The facilitator sponsors gas fees (after initial approval)
* **Universal compatibility**: Works with any ERC-20 token
* **One-time approval**: Users approve Permit2 once, then use signatures for all future payments
* **Broader token support**: Supports tokens that don't implement EIP-3009

**When to use Permit2:**
- Your token doesn't implement EIP-3009
- You want to support any ERC-20 token
- Users are willing to do a one-time approval transaction (paying gas once)

**When to use EIP-3009:**
- Your token implements EIP-3009 (like USDC)
- You want zero-friction payments with no approval step
...
x402 supports both EIP-3009 and Permit2 to provide flexibility:
...
3. **Best user experience**: Works immediately without any setup

**Permit2 Benefits:**
1. **Universal token support**: Works with any ERC-20 token, not just EIP-3009 tokens
2. **One-time approval**: After initial approval, all future payments are gasless signatures
3. **Ecosystem standard**: Leverages Uniswap's widely-adopted Permit2 contract
