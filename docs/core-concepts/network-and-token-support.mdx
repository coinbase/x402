...
#### x402.org Facilitator (Default Testnet)
...
Multiple production-ready facilitators are available supporting various networks including Base, Solana, Polygon, Avalanche, and more. When choosing a facilitator, consider:

* **Supported networks and tokens**: Facilitators vary in chain support
* **Compliance features**: Some offer KYT/OFAC compliance checks
* **API requirements**: Some require API keys, others are permissionless
* **Self-hosting options**: Some facilitators can be self-hosted for full control

See the [x402 Ecosystem](https://www.x402.org/ecosystem?category=facilitators) to explore available options and their documentation.
