...
* **EVM**: Any ERC-20 token via EIP-3009 (recommended) or Permit2 (universal fallback)
* **Solana**: Any SPL or token-2022 token

**Important**: Facilitators support networks, not specific tokens — any ERC-20 token works on EVM networks (via EIP-3009 or Permit2), and any SPL/token-2022 token works on Solana, for the facilitators that support those networks.

#### EVM: Asset Transfer Methods

x402 supports two methods for transferring ERC-20 tokens on EVM networks:

**EIP-3009 (Recommended)**
...
* **No approval required**: Payments work immediately without setup
* **Secure payments**: Transfers are authorized by cryptographic signatures

**Permit2 (Universal Fallback)**

For tokens that don't support EIP-3009, x402 uses [Uniswap's Permit2](https://github.com/Uniswap/permit2) contract:

* **Works with any ERC-20 token**: Universal compatibility
* **One-time approval**: Users approve Permit2 once per token
* **Signature-based transfers**: Similar security to EIP-3009
* **Gasless after approval**: Facilitator sponsors gas for transfers
...
**For EIP-3009 Tokens (Recommended)**
...
**For Non-EIP-3009 Tokens (Permit2)**

To use any ERC-20 token via Permit2, specify the `assetTransferMethod` in your price configuration:
...
```typescript
{
  "GET /api/resource": {
    accepts: [{
      ...
      network: "eip155:8453",
      price: {
        amount: "1000000",  // Token amount in atomic units
        asset: "0xYourTokenAddress",
        extra: {
          assetTransferMethod: "permit2",  // Use Permit2 for this token
        },
      },
    }],
  },
...
When using Permit2, clients must approve the Permit2 contract once before making payments:
...
After approval, payments work like EIP-3009 - clients sign payment authorizations and the facilitator handles gas.
...
#### Choosing Between EIP-3009 and Permit2

**Use EIP-3009 when:**
* Your token implements `transferWithAuthorization` (e.g., USDC, EURC)
* You want the simplest user experience (no approval step)
* You're using default stablecoins on supported networks

**Use Permit2 when:**
* Your token doesn't support EIP-3009
* You need to accept any ERC-20 token
* You're willing to require a one-time approval from users

Both methods provide:
* **Gas abstraction**: Buyers don't need native tokens (ETH, etc.) for gas
* **Signature-based authorization**: Secure off-chain payment signing
* **Universal facilitator support**: Works with any x402 facilitator
...
* Any ERC-20 token works via EIP-3009 (recommended) or Permit2 (universal fallback)
* Use price strings for USDC or TokenAmount for custom tokens
* Permit2 requires one-time approval but works with any ERC-20 token
