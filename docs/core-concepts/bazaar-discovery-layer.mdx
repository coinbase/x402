...
The Bazaar provides a `/list` endpoint that returns all x402-compatible services registered with facilitators that support the Bazaar extension. Services are discoverable when they include the bazaar extension in their route configuration.
