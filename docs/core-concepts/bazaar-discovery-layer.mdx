...
The Bazaar provides a `/list` endpoint that returns all x402-compatible services registered with facilitators that support the Bazaar extension. Services are automatically opted-in when they enable the bazaar extension with `discoverable: true`, making discovery frictionless for sellers.
