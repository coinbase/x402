...
If a resource advertises **`scheme: "batch-settlement"`** (EVM micropayments with escrow and **off-chain** vouchers), you must register the **`BatchSettlementEvmScheme`** on **`eip155:*`** in addition to **`ExactEvmScheme`** / **`UptoEvmScheme`**. The client deposits into escrow when needed, signs vouchers per request, and the seller redeems value **onchain** in batches—see **[Batch settlement](/core-concepts/batch-settlement)**. Python client support is not available yet.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    ```
    Full example: [`examples/typescript/clients/batch-settlement`](https://github.com/x402-foundation/x402/tree/main/examples/typescript/clients/batch-settlement).
  </Tab>
  <Tab title="Go">
    ```go
    import (
    ...
    ```
    Full example: [`examples/go/clients/batch-settlement`](https://github.com/x402-foundation/x402/tree/main/examples/go/clients/batch-settlement).
  </Tab>
  <Tab title="Python">
    Register **`BatchSettlementEvmScheme`** when Python support ships; use TypeScript or Go today.
  </Tab>
</Tabs>
