...
interface RouteConfig {
  accepts: Array<{
    ...
    price: string | {         // Price in dollars (e.g., "$0.01") or TokenAmount
      amount: string;         // Token amount in atomic units
      asset: string;          // Token contract address
      extra?: {
        assetTransferMethod?: "permit2";  // Use Permit2 for non-EIP-3009 tokens
      };
    };
...
By default, x402 uses EIP-3009 for token transfers (works with USDC and other EIP-3009 tokens). For tokens that don't support EIP-3009, you can use Permit2:
          ...
          "GET /custom-token": {
            accepts: [{
              ...
              price: {
                ...
                extra: {
                  assetTransferMethod: "permit2",  // Use Permit2
                },
              },
            }],
          },
...
**Note:** Clients using Permit2 must approve the Permit2 contract once before making payments. See [Network & Token Support](/core-concepts/network-and-token-support) for details.
