...
#### 4. Start the x402 MCP Server

In another terminal, start the Go MCP server example:

```bash
cd examples/go/servers/mcp
go run .
```
...
| `RESOURCE_SERVER_URL` | The base URL of the paid API (TypeScript) | TypeScript only |
| `ENDPOINT_PATH` | The specific endpoint path (e.g., `/weather`) (TypeScript) | TypeScript only |
| `MCP_SERVER_URL` | The URL of the MCP server (Go) | Go only |
...
func main() {
    ...
    if evmPrivateKey == "" {
        panic("EVM_PRIVATE_KEY environment variable is required")
    }
        ...
        mcp.Options{
            ...
            OnPaymentRequested: func(context mcp.PaymentRequiredContext) (bool, error) {
                ...
                fmt.Printf("Payment required: %s %s on %s\n", 
                    price.Amount, price.Asset, price.Network)
                return true, nil
            },
        },
    )
...
The MCP server exposes a tool that, when called, fetches data from a paid API endpoint. If the endpoint requires payment, the x402 wrapper automatically handles the payment handshake:
...
// Create x402 MCP client with multiple scheme registrations
x402Mcp := mcp.NewX402MCPClientFromConfig(
    adapter,
    []mcp.SchemeRegistration{
        {
        ...
        },
        {
            ...
            Client:  svm.NewExactSvmScheme(svmSigner),
        },
    },
    mcp.Options{},
)
...
* **MCP server (this implementation)**: Acts as a bridge, handling payment via x402 SDK and exposing tools to MCP clients.
...
* [See the TypeScript example in the repo](https://github.com/coinbase/x402/tree/main/examples/typescript/clients/mcp)
* [See the Go example in the repo](https://github.com/coinbase/x402/tree/main/examples/go/clients/mcp)
