...
For production services that need stronger guarantees — immutable on-chain attestations, DNS-based verification, temporal anchoring, or key lifecycle management (expiration, rotation, revocation) — ecosystem partners offer additional trust layers on top of `did.json`. See the [x402 Ecosystem page](https://www.x402.org/ecosystem) for reputation standards that integrate with the offer-receipt extension.
...
Ecosystem partners (see the [x402 Ecosystem page](https://www.x402.org/ecosystem)) build on these artifacts to provide reputation systems, trust scoring, and other value-added services.
