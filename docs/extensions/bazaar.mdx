...
* **For Buyers (API Consumers)**: Programmatically discover available x402-enabled services (HTTP APIs and MCP tools), understand their capabilities, pricing, and schemas
...
The Bazaar supports both **HTTP endpoints** and **MCP (Model Context Protocol) tools**, enabling discovery of traditional REST APIs and AI-native tool servers.
...
**Query Parameters:**
- `type` (optional): Filter by protocol type - `"http"` for HTTP endpoints or `"mcp"` for MCP tools
...
Add the bazaar extension to your route configuration to make your API discoverable. The Bazaar supports both HTTP endpoints and MCP tools.

#### Adding Metadata for HTTP Endpoints
...
For MCP (Model Context Protocol) tool servers, use the `toolName` field to declare your tools for discovery:
  ...
  <Tab title="TypeScript">
    ...
    const routes = {
      "POST /mcp": {
        ...
        network: "eip155:8453",
        payTo: "0xYourAddress",
        description: "Financial analysis MCP tool",
        extensions: {
...
**MCP-specific fields:**
- `toolName` (required): The MCP tool name - presence of this field identifies the config as MCP
- `description` (optional): Human-readable tool description
- `transport` (optional): MCP transport type (`"streamable-http"` or `"sse"`). Defaults to `streamable-http` per the MCP spec
- `inputSchema` (required): JSON Schema for tool arguments
- `example` (optional): Example tool arguments
- `output` (optional): Output specification with example
