# Fix test-utils
sed -i 's/Partial,/PartialEq,/g' cathedral-arkhe/test-utils/src/mock_council.rs

# Fix shard-controller
sed -i 's/Some(lease) if lease.is_expired()/Some(ref lease) if lease.is_expired()/g' cathedral-arkhe/shard-controller/src/failover.rs
sed -i 's/current_lease.map/current_lease.clone().map/g' cathedral-arkhe/shard-controller/src/failover.rs

# Fix cem-adapter
cat << 'EOF2' > cathedral-arkhe/cem-adapter/src/lib.rs
