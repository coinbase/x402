...
/// Wrapper for FileType to implement Serialize/Deserialize
#[derive(Clone, Debug)]
struct SerFileType(FileType);

impl Serialize for SerFileType {
...
impl<'de> Deserialize<'de> for SerFileType {
        ...
        D: Deserializer<'de>,
    {
        ...
        match s.as_str() {
            "NamedPipe" => Ok(SerFileType(FileType::NamedPipe)),
            "CharDevice" => Ok(SerFileType(FileType::CharDevice)),
            "BlockDevice" => Ok(SerFileType(FileType::BlockDevice)),
            "Directory" => Ok(SerFileType(FileType::Directory)),
            "RegularFile" => Ok(SerFileType(FileType::RegularFile)),
            "Symlink" => Ok(SerFileType(FileType::Symlink)),
            "Socket" => Ok(SerFileType(FileType::Socket)),
            _ => Err(serde::de::Error::custom(format!("unknown FileType variant: {}", s))),
        }
    }
}
...
impl CathedralFS {
    pub fn new(theosis_threshold: f64) -> Self {
        ...
        let root = CathedralNode {
            ...
            parent: FUSE_ROOT_ID,
            name: String::from("/"),
            kind: SerFileType(FileType::Directory),
            size: 0,
            blocks: 0,
            atime: now,
            mtime: now,
...
impl Filesystem for CathedralFS {
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        // Find the matching node by iterating and cloning the info we need to avoid mutable borrow overlap
        let found = self.nodes.values().find(|n| n.parent == parent && n.name == name_str).cloned();
    ...
    fn read(
        ...
        let found = self.nodes.get(&ino).cloned();
        if let Some(node) = found {
            // Verificar Theosis
            if !node.check_theosis(self.theosis_threshold) {
                reply.error(libc::EPERM);
