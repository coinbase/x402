...
async function runTest() {
  ...
  if (!serverEvmAddress || !serverSvmAddress || !clientEvmPrivateKey || !clientSvmPrivateKey || !facilitatorEvmPrivateKey || !facilitatorSvmPrivateKey) {
    ...
    errorLog(' Optional: SERVER_STELLAR_ADDRESS, CLIENT_STELLAR_PRIVATE_KEY, FACILITATOR_STELLAR_PRIVATE_KEY for Stellar support');
    process.exit(1);
  }
  ...
  const facilitatorEnvMapping: Record<string, string> = {
    EVM_PRIVATE_KEY: 'FACILITATOR_EVM_PRIVATE_KEY',
    SVM_PRIVATE_KEY: 'FACILITATOR_SVM_PRIVATE_KEY',
    STELLAR_PRIVATE_KEY: 'FACILITATOR_STELLAR_PRIVATE_KEY',
  };
  ...
  for (const [facilitatorName, facilitator] of uniqueFacilitators) {
    ...
    for (const envVar of requiredVars) {
      if (systemManagedVars.has(envVar)) {
        const sourceVar = facilitatorEnvMapping[envVar];
        if (sourceVar && !process.env[sourceVar]) {
          missing.push(envVar);
        }
        continue;
      }
