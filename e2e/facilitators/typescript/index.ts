...
if (process.env.HEDERA_ACCOUNT_ID && process.env.HEDERA_PRIVATE_KEY) {
  ...
  const hederaKey = HederaPrivateKey.fromStringECDSA(process.env.HEDERA_PRIVATE_KEY);
  ...
  hederaSigner = toFacilitatorHederaSigner({
    ...
    signAndSubmitTransaction: async (transactionBase64: string, _, network: string) => {
      ...
      if (HEDERA_NODE_URL) {
        client = HederaClient.forNetwork({
          [HEDERA_NODE_URL]: AccountId.fromString("0.0.3"),
        });
      } else if (network === "hedera:mainnet") {
        client = HederaClient.forMainnet();
...
if (hederaSigner) {
  facilitator.register(HEDERA_NETWORK as Network, new ExactHederaScheme(hederaSigner));
}
if (stellarSigner) {
