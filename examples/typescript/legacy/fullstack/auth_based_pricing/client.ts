...
import { kairos, base } from "viem/chains";
...
if (!CLIENT_SIM_PRIVATE_KEY) {
  console.error(
    "CRITICAL ERROR: Missing CLIENT_SIM_PRIVATE_KEY environment variable for client simulation. Check .env-local or .env file."
  );
  process.exit(1);
}
...
async function runClientDemo() {
  ...
  console.log(
    `[ClientSim] Using wallet address for simulation: ${clientWalletAddress}`
  );

  const serverBaseUrl = `http://localhost:${DEMO_SERVER_PORT}`;
  const chainId = kairos.id; // Chain ID for SIWE message (must match server if verified strictly)
  ...
  try {
    ...
    console.log(
      `[ClientSim]   Requesting nonce from ${serverBaseUrl}/auth/nonce...`
    );
    ...
    console.log(
      `[ClientSim]   Prepared SIWE message to sign:\n${messageToSign}`
    );

    // 1c. Client signs the SIWE message (simulating wallet interaction)
    const signature = await clientWalletAccount.signMessage({
      message: messageToSign,
    });
    console.log(
      `[ClientSim]   ✅ SIWE Message signed. Signature: ${signature.substring(
        0,
        10
      )}...`
    );
    ...
    console.log(
      `[ClientSim]   Verifying signature with server at ${serverBaseUrl}/auth/verify-siwe...`
    );
    ...
    const loginData = (await verifyResponse.json()) as {
      success?: boolean;
      token?: string;
      error?: string;
      message?: string;
  ...
  const fetchWithClientPayment = wrapFetchWithPayment(
    _fetch as any,
    clientWalletAccount
  );
  ...
  if (jwtToken) {
    ...
    try {
      const response = await fetchWithClientPayment(
        `${serverBaseUrl}/demo-weather`,
        {
          method: "GET",
      ...
      if (!response.ok) {
        ...
        console.error(
          `[ClientSim]   ❌ Error from server (authenticated call): ${response.status} - ${errText}`
        );
        // Log x-payment-response even on error, if present, for debugging x402 client issues
        if (x402RespHeader)
          console.error(
            "[ClientSim]   x-payment-response (on error):",
            decodeXPaymentResponse(x402RespHeader)
          );
        throw new Error(
          `Authenticated /demo-weather call failed: ${response.status}`
        );
      }

      const weatherData = await response.json();
      console.log(
        "[ClientSim]   ✅ Weather data (authenticated):",
        weatherData
      );
      if (x402RespHeader) {
        console.log(
          "[ClientSim]   ✅ x-payment-response (authenticated):",
          decodeXPaymentResponse(x402RespHeader)
        );
      }
    } catch (error: any) {
      console.error(
        "[ClientSim]   ❌ Error during authenticated /demo-weather call:",
        error.message
      );
    }
  } else {
  ...
  try {
    ...
    if (!response.ok) {
      const errText = await response.text();
      console.error(
        `[ClientSim]   ❌ Error from server (unauthenticated call): ${response.status} - ${errText}`
      );
      if (x402RespHeader)
        console.error(
          "[ClientSim]   x-payment-response (on error):",
      ...
      throw new Error(
        `Unauthenticated /demo-weather call failed: ${response.status}`
      );
    }

    const weatherData = await response.json();
    console.log(
      "[ClientSim]   ✅ Weather data (unauthenticated):",
      weatherData
    );
    if (x402RespHeader) {
      console.log(
        "[ClientSim]   ✅ x-payment-response (unauthenticated):",
        decodeXPaymentResponse(x402RespHeader)
      );
    }
  } catch (error: any) {
    console.error(
      "[ClientSim]   ❌ Error during unauthenticated /demo-weather call:",
      error.message
    );
  }
...
if (
  process.argv[1] &&
  fileURLToPath(`file://${process.argv[1]}`) === currentFilePath
) {
