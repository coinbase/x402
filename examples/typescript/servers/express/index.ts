...
app.get("/protected-currency", (req, res) => {
  res.json({ message: "Currency shorthand endpoint", timestamp: new Date().toISOString() });
});

app.get("/protected-eip3009", (req, res) => {
  res.json({ message: "EIP-3009 endpoint", timestamp: new Date().toISOString() });
});

app.get("/protected-eip2612", (req, res) => {
  res.json({ message: "EIP-2612 gas-sponsored endpoint", timestamp: new Date().toISOString() });
});

app.get("/protected-erc20", (req, res) => {
  res.json({ message: "ERC-20 approval gas-sponsored endpoint", timestamp: new Date().toISOString() });
});

app.get("/health", (req, res) => {
  res.json({ status: "ok", network: BASE_MAINNET });
});
